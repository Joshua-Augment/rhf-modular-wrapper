...
const yupValidation = yup.object({
  line: yup
    .string()
    .matches(/[TEST INPUT]|[NOT]/i, 'INPUT MUST BE "TEST INPUT" OR "NOT"')
...
export const CheckboxGroupExampleVertical = () => {
  ...
  useEffect(() => {
    fetch(imgFile)
      .then((data) => data.blob())
      .then((data) => {
        const file: any = new File([data], "1.png", {
          type: "image/png",
      ...
      {ready ? (
        ...
        >
          ...
          <Line
            name="line_number"
            type="number"
            label="Line Input (Number)"
            helperText="Line Input (Number)"
          />
          <DatePicker
            name="datepicker"
            label="Date Picker"
