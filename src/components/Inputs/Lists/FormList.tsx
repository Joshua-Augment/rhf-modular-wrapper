...
const FormList = (props: IList) => {
  return (
    <InputWrapper disableController empty={[]} type={props.type ?? "list"} {...props} noBorder>
  ...
  const bodygenerator = useMemo(() => {
    ...
    return fields.map((field, i) => {
      if (props.bodyTemplate !== undefined) {
        ...
        const bodyTemplateWithProps: any = React.cloneElement(props.bodyTemplate({ fields: field, index: i, ...props }), {
          fields: field,
          index: i,
          ...props,
        });
        ...
        return (
          <Row key={`fw-${props.name}-${i}`}>
            {props.showIndex === true && <Col g={1}>{i + 1}</Col>}
            {props.items.map((item: any, iT: number) => (
              <Col
                g={item.grid ?? 12 / (props.items.length + (props.showIndex ? 1 : 0) + (props.fixed !== true ? 1 : 0))}
                key={`fw-${props.name}-${i}-${iT}-iew`}
              >
                <InputChooser {...item} noBorder name={`${props.name}.${i}.${item.name}`} />
              </Col>
            ))}
            {props.fixed !== true && (
              <Col g={1}>
                <FaPlusSquare
                  style={{ ...iconStyle, color: props.maxItems && fields.length >= props.maxItems ? "olive" : "green" }}
                  onClick={() => {
                    if (!(props.maxItems && fields.length >= props.maxItems)) {
                      insert(i + 1, emptyRow);
                    }
                ...
                />{" "}
                <FaMinusSquare
                  style={{ ...iconStyle, color: fields.length > 1 ? "red" : "maroon" }}
                  onClick={() => {
                    if (fields.length > 1) {
                      remove(i);
                    }
