import React from "react";
import InputWrapper from "../../core/InputWrapper";
const Lines = (props) => {
    return (React.createElement(InputWrapper, Object.assign({}, props), (IWprops) => {
        var _a, _b, _c;
        console.log("[props] - ", props);
        return (React.createElement("textarea", { id: IWprops.name, className: (_b = (_a = IWprops === null || IWprops === void 0 ? void 0 : IWprops.customClasses) === null || _a === void 0 ? void 0 : _a.inputClassName) !== null && _b !== void 0 ? _b : "", name: IWprops.name, value: IWprops.value, onChange: IWprops.onChange, placeholder: IWprops.placeholder, rows: (_c = props.rows) !== null && _c !== void 0 ? _c : 3, cols: props.cols }));
    }));
};
export default Lines;
//# sourceMappingURL=Lines.js.map