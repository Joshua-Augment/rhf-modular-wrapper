...
const YesNo = (props) => {
    return (React.createElement(InputWrapper, Object.assign({ type: props.yesno }, props, { id: `${props.name}`, noLabel: true, noBorder: true, customClasses: { wrapperClassName: 'form-check' } }),
        React.createElement(_YesNo, Object.assign({}, props))));
};
const _YesNo = (props) => {
    ...
    const buttonGenerator = (label, valueOfButton, extHandler, ButtonElem, color, key) => {
        return React.createElement(ButtonElem, { className: props.inputClass, style: Object.assign({}, props.inputStyle), active: valueOfButton === props.value, key: key !== null && key !== void 0 ? key : `yng-${props.name}-${valueOfButton}`, type: "button", onClick: () => buttonHandler(valueOfButton, extHandler), bgColor: color }, label);
    };
    return React.createElement("div", { style: Object.assign(Object.assign({ display: 'flex', width: 100 * (2 + (props.otherOptions ? props.otherOptions.length : 0)) }, props.wrapperStyle), props.style), className: `${props.className} ${props.wrapperClass}` },
